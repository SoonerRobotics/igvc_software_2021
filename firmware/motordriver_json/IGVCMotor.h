...
class IGVCMotor {
    public:
        ...
        void update() {
            ...
            curOutput += output;
            curOutput = clamp(curOutput, -MAX_OUTPUT, MAX_OUTPUT);

            // printf("[%f,%f,%f\n", this->targetSpeed, this->pulses / (float)PULSES_PER_REV * 2.0 * PI * LINEAR_PER_REV * MOTOR_UPDATE_RATE, curOutput);
    ...
    private:
        ...
        float updatePID(float target_state, float cur_state) {            
            ...
            slope = (this->error - this->last_state) / dt;
